...
# Scale Modifiers
_ACC_SCLR_2G = const(16384.0)
_ACC_SCLR_4G = const(8192.0)
_ACC_SCLR_8G = const(4096.0)
_ACC_SCLR_16G = const(2048.0)

_GYR_SCLR_250DEG = const(131.0)
_GYR_SCLR_500DEG = const(65.5)
_GYR_SCLR_1000DEG = const(32.8)
_GYR_SCLR_2000DEG = const(16.4)

# Pre-defined ranges
_ACC_RNG_2G = const(0x00)
_ACC_RNG_4G = const(0x08)
_ACC_RNG_8G = const(0x10)
_ACC_RNG_16G = const(0x18)

_GYR_RNG_250DEG = const(0x00)
_GYR_RNG_500DEG = const(0x08)
_GYR_RNG_1000DEG = const(0x10)
_GYR_RNG_2000DEG = const(0x18)

# MPU-6050 Registers
_PWR_MGMT_1 = const(0x6B)

_ACCEL_XOUT0 = const(0x3B)

_TEMP_OUT0 = const(0x41)

_GYRO_XOUT0 = const(0x43)
...
class MPU6050:
    ...
    def _read_data(self, register) -> tuple[int | None, int | None, int | None]:
        fail_count = 0
        while fail_count < _maxFails:
            try:
                sleep_ms(10)
    ...
    def read_accel_data(self, g = False) -> tuple[int, int, int]:
        ...
        x = accel_data[0] / scaler
        y = accel_data[1] / scaler
        z = accel_data[2] / scaler
    ...
    def read_gyro_data(self) -> tuple[float, float, float]:
        gyro_data = self._read_data(_GYRO_XOUT0)
        ...
        x = gyro_data[0] / scaler
        y = gyro_data[1] / scaler
        z = gyro_data[2] / scaler

...
def main():
    ...
    parser.add_argument(
        ...
        "--port",
        type=str,
        help="The port where the chip is plugged in",
        required=True,
    )
    parser.add_argument(
        "--firmware", type=str, help="The relative path to the firmware"
    )
    ...
    firmware = (
        ...
        else os.path.join(os.path.dirname(__file__), FIRMWARE_FILENAME)
    )

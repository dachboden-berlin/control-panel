...
class LEDStrip(BaseLEDStrip, Fixture):
    def __init__(
        ...
        *,
        ...
        self._neopixels: neopixel.NeoPixel = neopixel.NeoPixel(
            Pin(pin, Pin.OUT), length
        )
        self._use_compression = use_compression
        self._animation: Generator[bytearray, None, None] | None = None
        self._primary_animation_color: list[int] = primary_animation_color or [
            100,
            0,
            0,
        ]
        self._secondary_animation_color: list[int] = secondary_animation_color or [
            0,
            100,
            0,
        ]
    ...
    def _parse_animation_data(self, animation_data: bytes | memoryview):
        assert len(animation_data) == _TOTAL_ANIM_BYTES, (
            f"Total number of bytes must be {_TOTAL_ANIM_BYTES}"
        )
        animation_index: int = animation_data[_ANIM_INDEX_OFFSET] - 1
        assert animation_index < len(self.ANIMATIONS), (
            f"Animation index {animation_index} outside range of animations"
        )
        animation: Animation = self.ANIMATIONS[animation_index]
        self.update_rate_ms: int = int(
            1000 / self.decode_update_rate(animation_data[_UPDATE_RATE_OFFSET])
        )
        animation_speed: float = self.decode_update_rate(
            animation_data[_ANIM_SPEED_OFFSET]
        )
        primary_color: tuple[int, int, int] = (
            animation_data[_PRIMARY_COLOR_OFFSET],
            animation_data[_PRIMARY_COLOR_OFFSET + 1],
            animation_data[_PRIMARY_COLOR_OFFSET + 2],
        )
        secondary_color: tuple[int, int, int] = (
            animation_data[_SECONDARY_COLOR_OFFSET],
    ...
    def _parse_pixel_data(self, pixel_data: bytes | memoryview):
        if not self._use_compression:
            assert len(pixel_data) == 3 * len(self._neopixels), (
                "length of pixel data must be 3 times the number of pixels"
            )
            self._neopixels.buf = bytearray(pixel_data)
        else:
            assert len(pixel_data) == len(self._neopixels), (
                "length of pixel data must be equal to the number of pixels"
            )

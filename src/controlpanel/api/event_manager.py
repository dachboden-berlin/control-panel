...
class EventManager:
    ...
    async def _ping(self, ip: str, pings: int, timeout: float) -> None:
        ...
        print(
            f"Received {len(list(r for r in reply_times if r != float('inf')))}/{len(reply_times)} pings. "
    ...
    async def _poll_and_collect(self, timeout=3.0) -> list[dict[str, Any]]:
        ...
        while True:
            ...
            if remaining <= 0:
                break
            try:
                reply: dict[str, Any] = await asyncio.wait_for(
                    self._reply_queue.get(), timeout=min(remaining, 0.1)
                )
    ...
    def _handle_artpoll_replies(self, replies: list[dict[str, Any]]) -> None:
        collected_mac_addresses: set[str] = (
            set()
        )  # a set of all MAC addresses from nodes that replied to our poll
        ...
        for esp in self._nodes:
            ...
            if esp.mac not in collected_mac_addresses:
                ...
                print(
                    f"ESP '{esp.name}' failed to reply! ({esp.subsequent_missed_replies} missed repl{'ies' if esp.subsequent_missed_replies > 1 else 'y'})"
                )
    ...
    def instantiate_devices(
        self,
        libs: Iterable[ModuleType],
        *,
        assign_sequential_universes: bool = False,
        start_universe: int = 5000,
    ) -> None:
        ...
        def find_class_in_modules(
            modules: Iterable[ModuleType], name: str
        ) -> type | None:
        ...
        with importlib.resources.open_binary(
            "controlpanel.shared", self.DEVICE_MANIFEST_FILENAME
        ) as file:
        ...
        for node_name, node_config in manifest.items():
            ...
            for device_name, (class_name, phys_kwargs, dummy_kwargs) in node_config[
                ...
                if cls is None:
                    print(
                        f"Failed to find {class_name} in any of these modules: {[lib.__name__ for lib in libs]}"
                    )
                    continue
                if (
                    issubclass(cls, Fixture)
                    and assign_sequential_universes
                    and kwargs.get("universe") is None
                ):
                    kwargs["universe"] = universe
                    print(
                        f"Assigned universe {universe} to {kwargs.get('name', '<UNKNOWN>')}."
                    )
                    universe += 1
                filtered_kwargs = {
                    key: value
                ...
                try:
                    if issubclass(cls, Fixture):
                        device = cls(
                            self._artnet, self.loop, esp, device_name, **filtered_kwargs
                        )
                    ...
                    print(
                        f"Type Error raised when instantiating {filtered_kwargs.get('name')}."
                    )
                    raise
        ...
        self.devices = {
            name: device for esp in self._nodes for name, device in esp.devices.items()
        }
        self._sensor_dict = {
            name: device
    ...
    def _parse_dmx(
        self, reply: dict[str, Any], sender: tuple[str, int], ts: float
    ) -> None:
        ...
        if fixture:
            print(
                f"Receiving ArtDMX event from {sender[0]} to fixture {fixture.name}: {values}"
            )
        else:
            print(
                f"Receiving ArtDMX event from {sender[0]} to universe {reply.get('Universe')}: {reply.get('Data')}"
            )

    def _parse_artpollreply(
        self, reply: dict[str, Any], sender: tuple[str, int], ts: float
    ) -> None:
    ...
    def _parse_artcmd(
        ...
        if self.print_incoming_artcmd_packets:
            print(
                f"Receiving ArtCommand event from {sender[0]}: {reply.get('Command')}"
            )
        if reply.get("Command") == "RETURN_PING":
            self.loop.call_soon_threadsafe(
                self._ping_queue.put_nowait, reply.get("Command")
            )

    def _parse_op(
        self, sender: tuple[str, int], ts: float, op_code: OpCode, reply: dict[str, Any]
    ) -> None:
        match op_code:
            ...
            case _:
                try:
                    print(
                        f"Received an {OpCode(op_code).name} packet from "
    ...
    def _get_ip_from_name_or_ip(self, name_or_ip: str) -> str | None:
        ...
        try:
            ...
            try:
                ip: str | None = next(
                    (esp for esp in self._nodes if esp.name == name_or_ip)
                ).ip
                if not ip:
                ...
                print(
                    f"{name_or_ip} is neither a valid IPv4 address nor the name of a registered ArtNet node"
                )
                return None

    def _sensor_autocomplete(self, text: str) -> tuple[int, list[Autocomplete.Option]]:
        return 0, [
            Autocomplete.Option(sensor, "")
            for sensor in self._sensor_dict.keys()
            if sensor.startswith(text)
        ]

    def _mute_autocomplete(self, text: str) -> tuple[int, list[Autocomplete.Option]]:
        return 0, [
            Autocomplete.Option(sensor.name, "")
            for sensor in self._sensor_dict.values()
            if sensor.name.startswith(text) and not sensor.muted
        ]
    ...
    def _unmute_autocomplete(self, text: str) -> tuple[int, list[Autocomplete.Option]]:
        return 0, [
            Autocomplete.Option(sensor.name, "")
            for sensor in self._sensor_dict.values()
            if sensor.name.startswith(text) and sensor.muted
        ]

    @console_command(
        is_cheat_protected=True, autocomplete_function=_unmute_autocomplete
    )
    ...
    @console_command(
        is_cheat_protected=True, autocomplete_function=_sensor_autocomplete
    )
    ...
    def send_artcmd(self, cmd: str, name_or_ip: str | None = None) -> None:
        ...
        ip = (
            self._get_ip_from_name_or_ip(name_or_ip)
            if name_or_ip
    ...
    def send_artdmx(self, device_name_or_universe: str | int, *values: int) -> None:
        ...
        if type(device_name_or_universe) is str:
            ...
            type(device_name_or_universe) is int
            and 0 <= device_name_or_universe < 2**15
        ):
    ...
    def fire_event(
        ...
        *,
        ...
        print(
            f"{'Firing event:':<16}{event.source:<20} -> {event.action:<20} -> {str(event.value):<20} from {event.sender}"
        )
        pg.event.post(
            pg.event.Event(
                CONTROL_PANEL_EVENT,
    ...
    async def _notify_subscribers(self, event: Event) -> None:
        for key_func in self.POSSIBLE_EVENT_TYPES:
            ...
            subscribers: list[Subscriber] = self._callback_register.get(
                Condition(source, name, value), []
            )
            for subscriber in subscribers:
                if not subscriber.allow_parallelism:
                    if subscriber.task is not None and not subscriber.task.done():
                        print(
                            f"[EventManager] Skipping {subscriber.callback.__name__}: still running."
                        )
                        continue
                print(
                    f"{'Event received: ':<16}{subscriber.callback.__module__.rsplit('.')[-1]}.{subscriber.callback.__name__}"
                )
    ...
    def subscribe(
        ...
        *,
        ...
        subscriber = Subscriber(
            callback, fire_once, allow_parallelism, requires_event_arg
        )

...
class Shaders:
    def __init__(
        ...
        shader_operations: list[tuple[int, str, dict]],
        *,
        texture_filter=moderngl.LINEAR,
    ):
        self.ctx: moderngl.Context = moderngl.create_context(require=300)
        self.textures: list[moderngl.Texture] = [
            self.ctx.texture(texture_size, 4) for texture_size in texture_sizes
        ]
        ...
        for target_buffer, shader_name, uniforms in shader_operations:
            if shader_name not in programs:
                ...
                self.shader_operations.append(
                    (target_buffer, programs[shader_name], uniforms)
                )

        quad_buffer_normal = self.ctx.buffer(
            data=array(
                "f",
                [
                    -1.0,
        ...
        quad_buffer_invert = self.ctx.buffer(
            data=array(
                "f",
                [
                    -1.0,
        ...
        for i, (_, program, _) in enumerate(self.shader_operations):
            quad_buffer = (
                quad_buffer_invert
                if i == len(self.shader_operations) - 1
                else quad_buffer_normal
            )
...
def get_shader_program(
    ...
    shader_parameters: dict[str, int | float],
) -> moderngl.Program:
...
def get_vertex_array(
    ctx: moderngl.Context, quad_buffer: moderngl.Buffer, program: moderngl.Program
